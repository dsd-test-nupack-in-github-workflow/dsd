...
struct DesignRunner {
    auto operator()(Specification const &spec, Local const &env, Condition condition, Handler handler, std::optional<DesignResult> restart_={}, double max_time=0) const {
        SignalRuntime l;
...
void render(Document &doc, Type<Specification> t) {
    ...
    doc.method(t, "evaluate", [](Specification spec, Local const &env) {
        spec.wobble_mutations = true;
        Designer d(std::move(spec));
